...
////////////////////////////////////////////////////////////////////////////////
/** domaincase_flmltX initialization function
 *
 * @param p_domn  \input set domain pointer with.
 * @param p_phi   \input set vector pointer with.
 */

void domaincase_flmltX::init(domain *p_domn) {
    ...
    if(domn->pram->heatloss == 0.0)
        domn->v.push_back(new dv_enth_flmltX(    domn, "enth",    true,  true ));
    else
        domn->v.push_back(new dv_enth_flmltX(    domn, "enth",    false,  true ));
    ...
    if (domn->pram->Lsoot) {
        ...
        //        domn->v.push_back(new dv_soot_CQMOM(domn, "M"+to_string(k)+","+'0', true, true ));          // first s column: M00, M10, M20, etc.
        //    }
        //    for (int k=1; k<domn->pram->nsvar_s; k++) {
        //        for (int j=0; j<domn->pram->nsvar_v/2; j++) {
        //            domn->v.push_back(new dv_soot_CQMOM(domn, "M"+to_string(j)+","+to_string(k), true, true ));       // other s columns: M01, M11, M02, M12, etc.
        //        }
        //    }
        //}   // end CQMOM
    ...
    for(int i=0; i<domn->ngrd; i++){
        domn->mixf->d[i] = (domn->pos->d[i] - domn->posf->d[0])/(domn->posf->d[domn->ngrd] - domn->posf->d[0]);
        domn->enth->d[i] = domn->strm->h0*(1.0-domn->mixf->d[i]) + domn->strm->h1*domn->mixf->d[i];   // redundant, this is set in getProdOfCompleteComb
    }
    ...
    for(int i=0; i<domn->ngrd; i++) {
        if ( domn->pram->Lignition ) {
            ...
            domn->strm->getMixingState(domn->mixf->d[i], ysp, domn->enth->d.at(i), domn->temp->d.at(i));
        }
        else {
            /* products of combustion for equilibrium initial state */
            domn->strm->getProdOfCompleteComb(domn->mixf->d[i], ysp, domn->enth->d.at(i), domn->temp->d.at(i));
        }
        for(int k=0; k<nsp; k++)
...
void domaincase_flmltX::setGasStateAtPt(const int &ipt) {
 ...
 *  These should not be transported. Those are already set.
 */
void domaincase_flmltX::setCaseSpecificVars() {
 ...
 *  These should not be transported. Those are already set.
 */
void domaincase_flmltX::setCaseSpecificVars_cvode(const int &ipt) {
