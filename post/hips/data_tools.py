...
def get_data_realization(DI, itime, iRlz, nGrid) :
    """
    ...
    nGrid: input: number of parcels per realization
    """
...
def get_nRlz(DI, nGrid) :

    fname = DI['cdir']+"data_py/data_py_00000.npy"

    data = np.load(fname)

    return int(len(data[:,0])/nGrid)
